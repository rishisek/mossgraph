...
void StickerSheet::changeMaxStickers(unsigned max){
    ...
    unsigned *yt = new unsigned[max];
    unsigned *xt = new unsigned[max];
...
int StickerSheet:: setStickerAtLayer(Image & Sticker, unsigned layer1,unsigned int x1, unsigned int y1){
    if (sticker_ct < sticker_max){
        if (layer[layer1] == 0){
            stickers[layer1] = new Image(Sticker);
            x[layer1] = x1;
            y[layer1] = y1;
            sticker_ct++;
            layer[layer1] = 1;
